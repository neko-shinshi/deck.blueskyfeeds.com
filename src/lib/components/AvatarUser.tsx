...
export default function AvatarSelf ({avatar, uri, size=70}:{avatar:string, uri:string, size:number}) {
    return <div className="w-12 h-12 aspect-square hover:blur-xs">
        <a href={`https://bsky.app/profile/${uri}`}>
            {
                avatar? <Image
                    height={size}
                    width={size}
